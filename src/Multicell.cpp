...
extern int *num_cell, *num_var, *num_par, *number_G_gj;
...
//				y_temp[i * num_var[0] + smc_ca]		=	((y[i * num_var[0] + smc_ca] * alpha * par[i * num_par[0] + rhocell] / (interval)) - par[i * num_par[0] + I_serca] + par[i * num_par[0] + I_srleak] + Q_ip3r[i] * beta * par[i * num_par[0] + p_ip3r] * y_converge[i * num_var[0] + smc_SR_ca] +  beta * Q_ryr[i] * P2(y_converge[i * num_var[0] + smc_R10]) * y_converge[i * num_var[0] + smc_SR_ca] +  2 * par[i * num_par[0] + I_ncx] -par[i * num_par[0] + I_vocc] - par[i * num_par[0] + I_pmca]) / (Q_ip3r[i] * beta * par[i * num_par[0] + p_ip3r] + beta * Q_ryr[i] * P2(y_converge[i * num_var[0] + smc_R10]) + (alpha * par[i * num_par[0] + rhocell] / interval));
			...
			/* Membrane potential, Calcium and IP3 formation */
			...
			/* forward sweep */
				/* First cell */
					if (i == 0)
							{
							/* Membrane potential*/
							R2 = number_G_gj[0] * IPF_GJ* f_rt* (Sum_ion_cyt + 2 * (y[i * num_var[0] + smc_ca] + y[(i+1) * num_var[0] + smc_ca]));
							...
							TDMCC[i] = -R2 / TDMB[i];
							TDMD[i]	= ((y[i * num_var[0] + smc_vm] * C_m * 1e-3 / interval) + par[i * num_par[0] + E_K] * G_bkca * par[i * num_par[0] + p_kca] + G_vocc * y_converge[i * num_var[0] + smc_dl] *  y_converge[i * num_var[0] + smc_fl] * par[i * num_par[0] + E_Ca] + G_cacc * par[i * num_par[0] + p_cacc] * par[i * num_par[0] + E_Cl] - par[i * num_par[0] + I_ncx] + G_nsc * (par[i * num_par[0] + p_nscdag]) * par[i * num_par[0] + p_nscvm] * E_nsc - par[i * num_par[0] + I_pmca] - Ca_SD_P[0] * (y[i * num_var[0] + smc_ca] - y[(i+1) * num_var[0] + smc_ca]));
							TDMDD[i]= TDMD[i] / TDMB[i];

							/* IP3 concentration */
							TDMD_IP3[i]	= ((y[i * num_var[0] + smc_ip3] / interval) + (par[i * num_par[0] + r_hg] * PIP_2T / gamma_G));
							TDMB_IP3[i]	= ((K_degG + 1 / interval) + IP3_P[0]);
							TDMCC_IP3[i] = -IP3_P[0] / TDMB_IP3[i];
							TDMDD_IP3[i] = TDMD_IP3[i] / TDMB_IP3[i];

							/* Calcium concentration */
							S2 = number_G_gj[0] * IPF_GJ* f_rt* (2 * (y[i * num_var[0] + smc_ca] + y[(i+1) * num_var[0] + smc_ca]));
							TDMB_Ca[i] = (Q_ip3r[i] * beta * par[i * num_par[0] + p_ip3r] + beta * Q_ryr[i] * P2(y_converge[i * num_var[0] + smc_R10]) + (alpha * par[i * num_par[0] + rhocell] / interval)) + Ca_SD_P[0];
							TDMCC_Ca[i] = -Ca_SD_P[0] / TDMB_Ca[i];
							TDMD_Ca[i]	= ((y[i * num_var[0] + smc_ca] * alpha * par[i * num_par[0] + rhocell] / (interval)) - par[i * num_par[0] + I_serca] + par[i * num_par[0] + I_srleak] + Q_ip3r[i] * beta * par[i * num_par[0] + p_ip3r] * y_converge[i * num_var[0] + smc_SR_ca] +  beta * Q_ryr[i] * P2(y_converge[i * num_var[0] + smc_R10]) * y_converge[i * num_var[0] + smc_SR_ca] +  2 * par[i * num_par[0] + I_ncx] -par[i * num_par[0] + I_vocc] - par[i * num_par[0] + I_pmca] - S2 * (y[i * num_var[0] + smc_vm] - y[(i+1) * num_var[0] + smc_vm]));
							TDMDD_Ca[i]= TDMD_Ca[i] / TDMB_Ca[i];
							}
				...
				/* Last cell */
							...
							{
							/* Membrane potential */
							R1 = number_G_gj[0] * IPF_GJ* f_rt* (Sum_ion_cyt + 2 * (y[i * num_var[0] + smc_ca] + y[(i-1) * num_var[0] + smc_ca]));
							TDMD[i]	= ((y[i * num_var[0] + smc_vm] * C_m*1e-3 / interval) + par[i * num_par[0] + E_K] * G_bkca * par[i * num_par[0] + p_kca] + G_vocc * y_converge[i * num_var[0] + smc_dl] * y_converge[i * num_var[0] + smc_fl] * par[i * num_par[0] + E_Ca] + G_cacc * par[i * num_par[0] + p_cacc] * par[i * num_par[0] + E_Cl] - par[i * num_par[0] + I_ncx] + G_nsc * (par[i * num_par[0] + p_nscdag]) * par[i * num_par[0] + p_nscvm] * E_nsc - par[i * num_par[0] + I_pmca]  - Ca_SD_P[0]* (y[i * num_var[0] + smc_ca] - y[(i-1) * num_var[0] + smc_ca]));
							...
							TDMD_IP3[i]	= ((y[i * num_var[0] + smc_ip3] / interval) + (par[i * num_par[0] + r_hg] * PIP_2T / gamma_G));
							TDMB_IP3[i]	= ((K_degG + 1 / interval) + IP3_P[0]);
							tdmm = 1.0 / (TDMB_IP3[i] + IP3_P[0] * TDMCC_IP3[i-1]);
							TDMDD_IP3[i] = (TDMD_IP3[i] + IP3_P[0] * TDMDD_IP3[i-1]) * tdmm;

							/* Calcium concentration */
							S1 = number_G_gj[0] * IPF_GJ* f_rt* (2 * (y[i * num_var[0] + smc_ca] + y[(i-1) * num_var[0] + smc_ca]));
							TDMB_Ca[i] = (Q_ip3r[i] * beta * par[i * num_par[0] + p_ip3r] + beta * Q_ryr[i] * P2(y_converge[i * num_var[0] + smc_R10]) + (alpha * par[i * num_par[0] + rhocell] / interval)) + Ca_SD_P[0];
							TDMD_Ca[i]	= ((y[i * num_var[0] + smc_ca] * alpha * par[i * num_par[0] + rhocell] / (interval)) - par[i * num_par[0] + I_serca] + par[i * num_par[0] + I_srleak] + Q_ip3r[i] * beta * par[i * num_par[0] + p_ip3r] * y_converge[i * num_var[0] + smc_SR_ca] +  beta * Q_ryr[i] * P2(y_converge[i * num_var[0] + smc_R10]) * y_converge[i * num_var[0] + smc_SR_ca] +  2 * par[i * num_par[0] + I_ncx] -par[i * num_par[0] + I_vocc] - par[i * num_par[0] + I_pmca] - S1 * (y[i * num_var[0] + smc_vm] - y[(i-1) * num_var[0] + smc_vm]));
							tdmm = 1.0 / (TDMB_Ca[i] + Ca_SD_P[0] * TDMCC_Ca[i-1]);
							TDMDD_Ca[i] = (TDMD_Ca[i] + Ca_SD_P[0] * TDMDD_Ca[i-1]) * tdmm;
							}
				...
				/* Inner cells */
							...
							R1 = number_G_gj[0] * IPF_GJ* f_rt* (Sum_ion_cyt + 2 * (y[i * num_var[0] + smc_ca] + y[(i-1) * num_var[0] + smc_ca]));
							R2 = number_G_gj[0] * IPF_GJ* f_rt* (Sum_ion_cyt + 2 * (y[i * num_var[0] + smc_ca] + y[(i+1) * num_var[0] + smc_ca]));
							TDMD[i]	= ((y[i * num_var[0] + smc_vm] * C_m * 1e-3 / interval) + par[i * num_par[0] + E_K] * G_bkca * par[i * num_par[0] + p_kca] + G_vocc * y_converge[i * num_var[0] + smc_dl] *  y_converge[i * num_var[0] + smc_fl] * par[i * num_par[0] + E_Ca] + G_cacc * par[i * num_par[0] + p_cacc] * par[i * num_par[0] + E_Cl] - par[i * num_par[0] + I_ncx] + G_nsc * (par[i * num_par[0] + p_nscdag]) * par[i * num_par[0] + p_nscvm] * E_nsc - par[i * num_par[0] + I_pmca]  - Ca_SD_P[0] * (2*y[i * num_var[0] + smc_ca] - y[(i+1) * num_var[0] + smc_ca] - y[(i-1) * num_var[0] + smc_ca]));
							...
							/* IP3 concentration */
							TDMD_IP3[i]	= ((y[i * num_var[0] + smc_ip3] / interval) + (par[i * num_par[0] + r_hg] * PIP_2T / gamma_G));
							TDMB_IP3[i]= ((K_degG + 1 / interval) + 2* IP3_P[0]);
							tdmm = 1.0/(TDMB_IP3[i] + IP3_P[0] * TDMCC_IP3[i-1]);
							TDMCC_IP3[i] = -IP3_P[0]*tdmm;
							TDMDD_IP3[i] = (TDMD_IP3[i] + IP3_P[0] * TDMDD_IP3[i-1]) * tdmm;

							/* Calcium concentration */
							S1 = number_G_gj[0] * IPF_GJ* f_rt* (2 * (y[i * num_var[0] + smc_ca] + y[(i-1) * num_var[0] + smc_ca]));
							S2 = number_G_gj[0] * IPF_GJ* f_rt* (2 * (y[i * num_var[0] + smc_ca] + y[(i+1) * num_var[0] + smc_ca]));
							TDMB_Ca[i] = (Q_ip3r[i] * beta * par[i * num_par[0] + p_ip3r] + beta * Q_ryr[i] * P2(y_converge[i * num_var[0] + smc_R10]) + (alpha * par[i * num_par[0] + rhocell] / interval)) + 2 * Ca_SD_P[0];
							TDMD_Ca[i]	= ((y[i * num_var[0] + smc_ca] * alpha * par[i * num_par[0] + rhocell] / (interval)) - par[i * num_par[0] + I_serca] + par[i * num_par[0] + I_srleak] + Q_ip3r[i] * beta * par[i * num_par[0] + p_ip3r] * y_converge[i * num_var[0] + smc_SR_ca] +  beta * Q_ryr[i] * P2(y_converge[i * num_var[0] + smc_R10]) * y_converge[i * num_var[0] + smc_SR_ca] +  2 * par[i * num_par[0] + I_ncx] -par[i * num_par[0] + I_vocc] - par[i * num_par[0] + I_pmca] - S1 * (y[i * num_var[0] + smc_vm] - y[(i-1) * num_var[0] + smc_vm]) - S2 * (y[i * num_var[0] + smc_vm] - y[(i+1) * num_var[0] + smc_vm]));
							tdmm = 1.0 / (TDMB_Ca[i] + Ca_SD_P[0] * TDMCC_Ca[i-1]);
							TDMCC_Ca[i] = -Ca_SD_P[0] * tdmm;
							TDMDD_Ca[i] = (TDMD_Ca[i] + Ca_SD_P[0] * TDMDD_Ca[i-1]) * tdmm;
							}
		...
		/* back substitution */
				...
				y_temp[(num_cell[0] - 1) * num_var[0] + smc_ca] = TDMDD_Ca[(num_cell[0] - 1)];
				...
				for (int i = num_cell[0] - 2; i >= 0; i--)
				{
					...
					y_temp[i * num_var[0] + smc_ca] = TDMDD_Ca[i] - y_temp[(i + 1) * num_var[0] + smc_ca] * TDMCC_Ca[i];
				}
